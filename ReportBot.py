...
def video_attack(proxies):
    video_url = ask_question("Linki aw videoya dane ka tawe reporti kai")
    ...
    print("")
    print_status("Skala krdn la videoka kawta esh!\n")
...
def profile_attack(proxies):
    username = ask_question("Nawy aw kasa bnwsa ka atawe Reporti kai")
    print(Style.RESET_ALL)
    if (len(proxies) == 0):
        for k in range(5):
    ...
    print("")
    print_status("Skala krdn law profile dast pe aka!\n")
...
def main():
    ...
    ret = ask_question("Atawet proxy bakar benit? [Y / N]")

    proxies = []

    if (ret == "Y" or ret == "y"):
        ret = ask_question("atawet proxyakan la internetawa wargri? [Y / N]")

        if (ret == "Y" or ret == "y"):
            print_status("Gathering proxy from the Internet! This may take a while.\n")
            proxies = find_proxies()
        elif (ret == "N" or ret == "n"):
            print_status("Please have a maximum of 14000 proxies in a file!")
    ...
    print("")
    print_status("1 - Reportkrdi Profile.")
    print_status("2 - Reportkrdni video.")
    report_choice = ask_question("danayakian halbzhera gulm")
    print("")

    if (report_choice.isdigit() == False):
        print_error("tenagashtm brakam")
        exit(0)
