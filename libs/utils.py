...
def parse_proxy_file(fpath):
    if (path.exists(fpath) == False):
        print("")
        print_error("File proxyaka nadozrayawa ! (lawanaya path bahala danabe gulm?)")
        print_error("la programakawa da axretawa")
        exit(0)
    ...
    if (len(proxies) > 2000):
        proxies = random.choices(proxies, 2000)
        
    print("")
    print_success(str(len(proxies)) + " Proxiaka install bw!")
