...
page_headers = {
    ...
    "Accept-Language": "kr-KR,tr;q=0.8,en-US;q=0.5,en;q=0.3",
...
def report_profile_attack(username, proxy):
    ...
    try:
        ...
        print_error("Connectionaka error bw! (FacebookRequestsError)")
        return

    if (res.status_code != 200):
        print_error("Connectionaka error bw! (STATUS CODE:", res.status_code, ")")
        return

    if ('["_js_datr","' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorJSDatr)")
        return
    
    try:
        js_datr = res.text.split('["_js_datr","')[1].split('",')[0]
    except:
        print_error("Connectionaka error bw! (CookieParsingError)")
        return
    ...
    try:
        ...
        print_error("Connectionaka error bw! (InstagramRequestsError)")
        return
    ...
    if ("datr" not in res.cookies.get_dict()):
        print_error("Connectionaka error bw! (CookieErrorDatr)")
        return
    
    if ('["LSD",[],{"token":"' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorLSD)")
        return

    if ('"__spin_r":' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorSpinR)")
        return

    if ('"__spin_b":' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorSpinB)")
        return

    if ('"__spin_t":' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorSpinT)")
        return

    if ('"server_revision":' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorRev)")
        return

    if ('"hsi":' not in res.text):
        print_error("Connectionaka error bw! (CookieErrorHsi)")
        return
    ...
    report_form = {
        ...
        "Field241164302734019_iso2_country_code": "KR",
        "Field241164302734019": "Kurdistan",
    ...
    try:
        ...
        print_error("Connectionaka error bw! (FormRequestsError)")
        return
    ...
    print_success("Ba sarkawtwi report kra!")
...
def report_video_attack(video_url, proxy):
    ...
    try:
        ...
        print_error("Connectionaka error bw! (FacebookRequestsError)", e)
        return
    ...
    report_form = {
        ...
        "Field1476905342523314_iso2_country_code": "KR",
        "Field1476905342523314": "Kurdistan",
        "support_form_id": "440963189380968",
        "support_form_hidden_fields": '{"423417021136459":false,"419623844841592":false,"754839691215928":false,"1476905342523314":false,"284770995012493":true,"237926093076239":false,"754839691215928"::false,"754839691215928"::false,"754839691215928"::false,"754839691215928"::false,"754839691215928"::false,"754839691215928"::false,"754839691215928":}',
        "support_form_fact_false_fields": "[]",
        "__user": "0",
        "__a": "1",
        "__dyn": "7xe6Fo4SQ1PyUhxOnFwn84a2i5U4e1Fx-ey8kxx0LxW0DUeUhw5cx60Vo1upE4W0OE2WxO0SobEa81Vrzo5-0jx0Fwww6DwtU6e",
